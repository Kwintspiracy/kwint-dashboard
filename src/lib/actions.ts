...
export async function getAgentsAction(): Promise<any[]> {
  ...
  const { data, error } = await supabase
    .from('agents')
    .select('id, name, slug, model, role, active, is_default, avatar_url, capabilities, requires_approval, task_context_template, system_agent, telegram_bot_token, telegram_bot_username, max_tokens_per_job, enabled_builtin_tools, created_at, updated_at')
    .eq('entity_id', entityId)
