...
import { getToolCalls } from '@/lib/queries'
...
export default function LogsPage() {
  ...
  async function load() {
    const data = await getToolCalls(200)
  ...
  return (
    <div className="space-y-5 max-w-7xl">
      <PageHeader title="Logs" count={filtered.length}>
        <button onClick={load} className="px-4 py-2 text-xs font-medium border border-neutral-700 text-neutral-400 rounded-lg hover:text-white hover:border-neutral-600 transition-colors">
          Refresh
        </button>
      </PageHeader>
